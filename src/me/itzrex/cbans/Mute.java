				...
				try {
					...
					if(target.hasPermission("cbans.shield")){
						...
						if(CustomBans.geInstance().isMySQL){
							plugin.getBanManager().mute(target.getName(), sender.getName(), reason);
					    ...
					    if(CustomBans.geInstance().isMySQL){
					    	...
					    	String reason1 = RexUtil.buildReason(args);
					    	CustomBans.geInstance().getBanManager().mute(args[0], sender.getName(), reason1);
