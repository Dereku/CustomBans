package me.itzrex.custombans.util;
...
public class Util{
    ...
    public static String getTime(long ms){
        ...
        if(ms / 31449600 > 0){
            ...
            if(years > 100) return "Навсегда";

            sb.append(years + (years == 1 ? " год " : " лет "));
            ms -= years * 31449600;
        }
        if(ms / 2620800 > 0){
            //Months
            long months = ms / 2620800;
            sb.append(months + (months == 1 ? " месяц " : " месяцев "));
            ms -= months * 2620800;
        }
        if(ms / 604800 > 0){
            //Weeks
            long weeks = ms / 604800;
            sb.append(weeks + (weeks == 1 ? " неделя " : " недель "));
            ms -= weeks * 604800;
        }
        if(ms / 86400 > 0){
            //Days
            long days = ms / 86400;
            sb.append(days + (days == 1 ? " день " : " дней "));
            ms -= days * 86400;
        }
        ...
        if(ms / 3600 > 0){
            //Hours
            long hours = ms / 3600;
            sb.append(hours + (hours == 1 ? " час " : " часов "));
            ms -= hours * 3600;
        }
        ...
        if(ms / 60 > 0){
            //Minutes
            long minutes = ms / 60;
            sb.append(minutes + (minutes == 1 ? " минута " : " минут "));
            ms -= minutes * 60;
        }

        if(ms > 0){
            //Seconds
            sb.append(ms + (ms == 1 ? " секунда " : " секунд "));
        }
